...
pub enum TcpBenchmarkPanelState {
    Idle,
    Running {
        start: Instant,
        duration: Duration,
...
pub struct TcpBenchmarkPanel {
    pub state: TcpBenchmarkPanelState,
    pub inputs: InputList,
    pub task: Option<TaskHandle<TcpBenchmarkEvent>>,
    pub log: Vec<String>,
}

impl PanelFooter for TcpBenchmarkPanel {
    fn footer_items(&self) -> Vec<FooterItem> {
        match &self.state {
            TcpBenchmarkPanelState::Idle => vec![
                FooterItem::new("enter", "run"),
                FooterItem::new("tab", "switch field"),
                FooterItem::new("space", "toggle"),
            ],
            TcpBenchmarkPanelState::Running { .. } => vec![],
            TcpBenchmarkPanelState::Done { .. } | TcpBenchmarkPanelState::Error(_) => {
                vec![
...
impl TcpBenchmarkPanel {
    pub fn new() -> Self {
        Self {
            state: TcpBenchmarkPanelState::Idle,
    ...
    pub fn is_busy(&self) -> bool {
        matches!(self.state, TcpBenchmarkPanelState::Running { .. })
    }

    pub fn on_key(&mut self, key: KeyCode) {
        match key {
            KeyCode::Enter => {
                if let TcpBenchmarkPanelState::Idle = self.state {
                    self.start();
                }
            }
            ...
            KeyCode::Esc => {
                if matches!(
                    ...
                    self.state = TcpBenchmarkPanelState::Idle;
                    self.log.clear();
                }
            }
            ...
            key => {
                if matches!(self.state, TcpBenchmarkPanelState::Idle) {
    ...
    pub fn poll_task(&mut self) {
        ...
        while let Some(event) = task.try_recv() {
            match event {
                TcpBenchmarkEvent::Done { client, server } => {
                    let (target_server, target_port) =
                        if let TcpBenchmarkPanelState::Running { server, port, .. } = self.state {
                            (server, port)
                        } else {
                            unreachable!()
                        };
                    ...
                    for line in format_tcp_benchmark_result(&client, true) {
                        self.log.push(line);
                    }
                    self.log.push(String::new());
                    self.log.push("> receiver (server)".to_string());
                    for line in format_tcp_benchmark_result(&server, false) {
                        self.log.push(line);
                    }
                    self.state = TcpBenchmarkPanelState::Done {
                        server: target_server,
                        port: target_port,
                    };
                }
                TcpBenchmarkEvent::Error(e) => {
                    self.log.push(format!("! {e}"));
                    self.state = TcpBenchmarkPanelState::Error(e);
                }
            }
        }
        ...
        if task.is_finished() {
            self.task = None;
            if matches!(self.state, TcpBenchmarkPanelState::Running { .. }) {
    ...
    pub fn draw(&self, frame: &mut Frame, area: Rect) {
        ...
        match &self.state {
            TcpBenchmarkPanelState::Idle => self.draw_idle(frame, inner),
            TcpBenchmarkPanelState::Running {
                start,
                duration,
                server,
                port,
            } => self.draw_running(frame, inner, *start, *duration, *server, *port),
            TcpBenchmarkPanelState::Done { server, port } => {
                self.draw_results(frame, inner, *server, *port)
            }
            TcpBenchmarkPanelState::Error(e) => self.draw_error(frame, inner, e),
        }
    }
    ...
    fn start(&mut self) {
        ...
        let server: Ipv4Addr =
            match parser::parse_server_ipv4(self.inputs.get_text("server").trim()) {
                ...
                Err(e) => {
                    self.state =
                        TcpBenchmarkPanelState::Error(format!("invalid server address: {e}"));
                    return;
                }
            };
        ...
        let port: u16 = match self.inputs.get_text("port").trim().parse() {
            Ok(p) => p,
            Err(_) => {
                self.state = TcpBenchmarkPanelState::Error(format!(
                    "invalid port: {}",
        ...
        let duration = match parser::parse_duration_min_1s(self.inputs.get_text("time").trim()) {
            Ok(d) => d,
            Err(e) => {
                self.state = TcpBenchmarkPanelState::Error(format!("invalid time: {e}"));
                return;
            }
        };
        ...
        let n_streams: u16 = match self.inputs.get_text("stream count").trim().parse::<u16>() {
            ...
            Ok(_) => {
                self.state =
                    TcpBenchmarkPanelState::Error("streams must be between 1 and 32".into());
                return;
            }
            Err(_) => {
                self.state = TcpBenchmarkPanelState::Error(format!(
        ...
        let verify_integrity = if self.inputs.get_toggle("verify") {
            if n_streams > 1 {
                self.state = TcpBenchmarkPanelState::Error(
        ...
        let (tx, rx) = mpsc::channel::<TcpBenchmarkEvent>();
        ...
        let thread = thread::spawn(move || match run_tcp_silent(params) {
            Ok((Some(client), Some(server))) => {
                let _ = tx_clone.send(TcpBenchmarkEvent::Done { client, server });
            }
            Ok(_) => {
                let _ = tx_clone.send(TcpBenchmarkEvent::Error(
                    "benchmark returned incomplete stats".into(),
                ));
            }
            Err(e) => {
                let _ = tx_clone.send(TcpBenchmarkEvent::Error(e.to_string()));
            }
        });

        self.state = TcpBenchmarkPanelState::Running {
            start: Instant::now(),
            duration,
            server,
            port,
