...
#[cfg(target_os = "linux")]
#[inline]
fn enable_timestamping(sock: &UdpSocket) -> bool {
    enable_so_timestampns(sock)
}

#[cfg(target_os = "macos")]
#[inline]
fn enable_timestamping(sock: &UdpSocket) -> bool {
    enable_so_timestamp(sock)
}

#[cfg(target_os = "linux")]
const TIMESTAMP_CMSG_TYPE: libc::c_int = libc::SO_TIMESTAMPNS;

#[cfg(target_os = "macos")]
const TIMESTAMP_CMSG_TYPE: libc::c_int = libc::SCM_TIMESTAMP;

// Unix timestamp receiver
#[cfg(unix)]
mod platform {
    use super::*;
    ...
    fn extract_kernel_ts(msg: &libc::msghdr, epoch_base_ns: u64) -> Option<u64> {
        unsafe {
            ...
            while !cmsg.is_null() {
                ...
                if hdr.cmsg_level == libc::SOL_SOCKET && hdr.cmsg_type == TIMESTAMP_CMSG_TYPE {
                    #[cfg(target_os = "linux")]
                    {
